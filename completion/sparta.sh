...
_sparta_launch() {
	...
	# TODO
	dirs="$(\ls --color=never -c1 ~/.cache/sparta/workspaces)"
...
_sparta() {
	...
	# check if we're completing 'sparta'
	...
	# if we're not completing 'sparta', then we're completing a subcommand
	case "$cmd" in
		init)
			COMPREPLY=()
			;;
